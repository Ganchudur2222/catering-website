    ...
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-light-gray)' }}>
        ...
        <div className="container" style={{ marginTop: '2rem' }}>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '2rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                {activeTab === 'orders' && (
                    <div>
                        <h3 className="mb-4" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Clock size={20} /> Incoming Orders</h3>
                ...
                {activeTab === 'history' && (
                    <div>
                        <h3 className="mb-4" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><CheckCircle size={20} /> Order History</h3>
                            ...
                            {menuItems.map(item => (
                                <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0', paddingBottom: '1rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                        <button className="btn" style={{ color: 'var(--color-text-light)' }} onClick={() => openModal(item)}><Edit size={18} /></button>
                                        <button className="btn" style={{ color: '#ef4444' }} onClick={() => handleDeleteItem(item.id)}><Trash2 size={18} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'settings' && (
                    <div>
                        <h3 className="mb-4" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Settings size={20} /> Site Settings</h3>
...
const OrderCard = ({ order, onUpdate, isHistory }) => (
    <div style={{ border: '1px solid #e2e8f0', padding: '1rem', borderRadius: '0.5rem', backgroundColor: isHistory ? '#f8fafc' : 'white' }}>
        ...
        <div style={{ marginTop: '1rem' }}>
            ...
            {isHistory && (
                <button
                    onClick={() => onUpdate(order.id, 'pending')}
                    className="btn"
                    style={{ fontSize: '0.8rem', padding: '0.5rem 1rem', border: '1px solid #cbd5e1' }}
                >
                    Mark as Pending
                </button>
            )}
